...
export async function refreshReservationProgramSnapshots(supabase, reservationNo, reservation, packages = []) {
  ...
  const safeBasisDate = DATE_RE.test(String(basisDate || '')) ? basisDate : null
  ...
  const [vendorRes, historyRes] = await Promise.all([
    ...
    safeBasisDate
      ? supabase
        ...
        .lte('effective_from', safeBasisDate)
        .or(`effective_to.is.null,effective_to.gte.${safeBasisDate}`)
        .or('is_deleted.is.null,is_deleted.eq.false')
  ...
  for (const component of snapshotInputs) {
    ...
    if ((component.sale_type || 'package') === 'single') {
      ...
      const vendorSettlePrice = Number(component.vendor_settle_unit_price) || Number(history?.vendor_settle_price ?? currentProgram?.vendor_settle_price ?? currentProgram?.unit_price ?? 0) || 0
      rows.push({
        ...
        snapshot_memo: Number(component.vendor_settle_unit_price) ? 'custom single component price' : history ? null : 'component single snapshot',
      })
      continue
    }
