...
src_path = r'C:\Users\USER\Downloads\예약 정산 관리\체험예약관리_v8.html'
dst_path = r'C:\Users\USER\roadnvill9\index.html'

with open(src_path, 'r', encoding='utf-8') as f:
    html = f.read()

# 1. Add Supabase CDN after font link
html = html.replace(
    '<link href="https://fonts.googleapis.com/css2?family=Noto+Sans+KR:wght@300;400;500;700&family=DM+Mono:wght@400;500&display=swap" rel="stylesheet">',
    '<link href="https://fonts.googleapis.com/css2?family=Noto+Sans+KR:wght@300;400;500;700&family=DM+Mono:wght@400;500&display=swap" rel="stylesheet">\n<script src="https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2"></script>'
)

# 2. Remove Cloudflare script wrapper
html = html.replace(
    '<script data-cfasync="false" src="/cdn-cgi/scripts/5c5dd728/cloudflare-static/email-decode.min.js"></script><script>',
    '<script>'
)

# 3. Remove Cloudflare email obfuscation link
html = re.sub(
    r'<a href="/cdn-cgi/l/email-protection"[^>]*>[^<]*</a>',
    'admin@experience.com',
    html
)
...
(async function _supabaseIntegration() {
  ...
  // ────────────────────────────────────────────────────────────────
  // initApp : Supabase에서 전체 데이터 로드 → in-memory 구조 채우기
  // ────────────────────────────────────────────────────────────────
    ...
    try {
      ...
      masterData.vendors = (vR.data||[]).map(v=>({
        ...
        programs:((v.vendor_programs||[]).map(p=>({
          progName:p.prog_name||p.progName||'',
      ...
      // reservations (Supabase snake_case → v8 camelCase)
      ...
      // master_config → platforms, drivers, bizNames
  ...
  window.saveMasterModal = async function() {
    ...
    if(type==='vendor'){
      ...
      body={zone:gv('mm-p-zone'),name:gv('mm-p-name'),pax_limit:parseInt(gv('mm-p-limit'))||null};
  ...
  window.savePkgProgModal = async function() {
    ...
    try {
      ...
      if(_pkgProgProgIdx>=0){
        const existing=pkg.programs[_pkgProgProgIdx];
        if(existing?._id){({error:err2}=await _sb.from('programs').update(progBody).eq('id',existing._id));}
      } else {
  ...
  window.saveVendorProgModal = async function() {
    ...
    const newP={progName,unitPrice,settleType};
    if(_vpProgIdx>=0) progs[_vpProgIdx]=newP; else progs.push(newP);
    const vpArr=progs.map(p=>({prog_name:p.progName,unit_price:p.unitPrice,settle_type:p.settleType}));
    try {
      if(vendor._id){
        const {error:err2}=await _sb.from('vendors').update({vendor_programs:vpArr}).eq('id',vendor._id);
        if(err2){alert('저장 실패: '+err2.message);return;}
      }
    } catch(e){alert('저장 실패: '+e.message);return;}
    closeModal('master-modal');
...
# 5. Insert Supabase block before </body>
html = html.replace('</body>', SUPABASE_BLOCK + '\n</body>')

with open(dst_path, 'w', encoding='utf-8') as f:
    f.write(html)

size_kb = os.path.getsize(dst_path) // 1024
print(f'Done! {dst_path}  ({size_kb} KB)')
