...
- `reservation_profit_adjustments`: 고객 혜택/추가 할인/보상 등 수익 조정 항목
...
- 고객 혜택, 추가 할인, 지원사업, 컴플레인 보상, 무료 제공 등은 수익 조정 항목으로 별도 기록한다.
- 예상 수익이 음수인 예약은 상세 검색과 예약 상세에서 확인할 수 있게 한다.
- 기존 예약의 과거 프로그램별 고객 판매가 배분은 정확히 복원할 수 없으므로, 마이그레이션 시점의 현재 기준정보로 초기 스냅샷을 생성한다.

예상 수익 계산 방향:

- 예상 수익 = 체험 판매금액 - 플랫폼 수수료 - 여행사 수수료 - 체험 업체 정산금 - 숙박 정산금 - 픽업 정산금 - 수익 조정금
- 수익 조정금은 항상 양수 금액으로 입력하고 예상 수익에서 차감한다.
- 예상 수익이 0 미만이면 `손실 예상`으로 표시한다.
