...
## 6. 2026-05-13 추가 확정 사항

### 6.1 사업비 관리는 운영에 필요한 상품운영비 중심으로 단순화

전체 행정 예산표의 국비/지방비/자부담, 상품고도화비, 컨설팅, 인건비, 운영비는 1차 운영 화면에 넣지 않는다.

1차 사업비 관리 대상은 상품운영비이며, 아래 항목을 계획 예산으로 관리한다.

| 구분 | 상품명 | 지원율 | 계획 인원 | 기준 단가 | 총 예산 |
|---|---|---:|---:|---:|---:|
| 상품운영비 | 금양연화 | 50% | 80명 | 115,000 | 9,200,000 |
| 상품운영비 | 삼베마을 | 40% | 250명 | 50,000 | 12,500,000 |
| 상품운영비 | 가보시더 | 50% | 80명 | 115,000 | 9,200,000 |
| 상품운영비 | 왔니껴 | 40% | 250명 | 50,000 | 12,500,000 |
| 상품운영비 | 암소해피2박3일 | 50% | 20명 | 200,000 | 4,000,000 |

사업비 관리 화면은 각 항목별로 계획 인원, 실제 이용 인원, 기준 단가, 총 예산, 사용액, 잔액, 집행률을 보여준다.

### 6.2 업체별 정산과 사업비 집행 현황은 분리

- 업체별 정산: 실제로 업체/숙소/픽업/플랫폼/여행사에 지급하거나 정산해야 하는 금액
- 사업비 관리: 어떤 상품운영비 예산을 얼마나 사용했는지 확인하는 현황

두 화면은 같은 예약 데이터를 볼 수 있지만 목적이 다르므로 하나로 합치지 않는다.

### 6.3 연계 패키지는 추후 기능으로 분리

현재 패키지는 업체 프로그램을 묶는 단일 패키지 구조를 우선 안정화한다.

추후에는 이미 만들어둔 패키지 여러 개를 다시 묶는 연계 패키지를 추가한다.
...
위 3개를 순서대로 묶어 `금양연화 + 일반체험 + 삼베마을 연계 패키지`처럼 운영할 수 있게 한다.

1차 구현에서는 연계 패키지 안에 또 다른 연계 패키지를 넣는 무한 중첩은 허용하지 않는다.

### 6.4 준비된 SQL

- `supabase_biz_budget_items_schema_20260513.sql`: 상품운영비 사업비 항목 테이블 및 초기 계획 데이터 생성

이 SQL은 로컬 화면 검증 후 Supabase SQL Editor에서 실행한다.
